...
%       srcfile.mask.txt .
%  extmask - Mask to be imposed on the internally created mask. 3D-matrix with 
%    image dimensions
%  dimensions of image
...
if (nargin > 1)
    for i=1:2:size(varargin,2)
        ...
        switch varargin{i}
            ...
            case 'phantom'
                phantom = varargin{i+1};
            case 'phantommask'
                phantommask = varargin{i+1};
            case 'flipBaxis'    % for NYU data, usually, flipBaxis 1; for MGH JCP data, flipBaxis 2
                flipbaxis = varargin{i+1};
            case 'swapAxis'
                swapaxis = varargin{i+1};
            otherwise
...
swapstr = '';
if (~isempty(swapaxis))
    b_table(swapaxis+1,:) = b_table(2:4,:);
    swapstr = [swapstr [num2str(swapaxis')]'];
end;
if (~isempty(flipbaxis))
    for i = 1:length(flipbaxis)
        b_table(flipbaxis(i)+1,:) = -b_table(flipbaxis(i)+1,:);
        swapstr = [swapstr 'f' num2str(flipbaxis(i))];
    end;
end;
if (~isempty(swapstr))
    fibfile = strrep(fibfile,'.fib',['.' swapstr '.fib']);
end;
...
if (phantommask)
    ...
    [mask,maskc,masks] = segmentbrain(img,maskfile,0,[],...
            floor(size(img,3)*2/5):floor(3/5*size(img,3)),extmask>0);
end;
...
%% find the shells
[bvalues,nmeasshell,shell] = bval2shells(b_table(1,:));
nshells = length(nmeasshell);
...
data2odf = radial_recon_matrix(odf_vertices, ...
    b_table2, edgefactor,'dcfestmethod','voronoi'); % theor, voronoi, jackson, 'pipe2'
if (output);fprintf(1, 'done\n');end
...
minIpsi0 = zeros(1, size(mask,1));
shellval = zeros(1, nshells);
...
for mm = 1:size(mask,1)
    ...
    shi = [];%max(find(dshell(2:end) < 0.1*dshell(1:(end-1)),1,'first'),2)+1;
    if (isempty(shi)), shi = nshells; end

    if (shj > 2 | shj >= shi) % image data saturated ... no use to convert
        shi = 1;shj = 1;
        odfTmp = 1e-8 * ones(2*odfHalfLength,1);
    else        
        % calculate the odf
        sel = (shell <= shi) & (shell >= shj);
        odfTmp = data2odf(:,sel) * data(sel);
    end;
...
if (sum(minIpsi0)>0)
    ...
    Ipsi0 = 1;
    if (phantom)
        Ipsi0 = prctile(max(odf,[],1)-min(odf,[],1),99);
    end;
end
vol = 1;
z0 = vol/Ipsi0;
...
for mm = 1:size(mask,1)
   ...
   if (odfVals(1) < 1e-4/z0+min_odf)
       odfVals(:) = 1e-4/z0+min_odf;
   end
   for ff = 0:maxfibers-1
      if (ff+1 <= length(pks))
...
max_odf = max(odf, [], 2)';
% save(fibfile, '-regexp', 'max_odf', 'report', '-mat', '-append');
