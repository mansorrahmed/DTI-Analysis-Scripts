...
dims = size(x);
dd = length(Nd);
if (ndims(x) < dd & Nd(3) < 1), fail 'input signal has too few dimensions', end
if any(dims(1:min(dd,length(dims))) ~= Nd(1:min(dd,length(dims)))), fail 'input signal has wrong size', end
