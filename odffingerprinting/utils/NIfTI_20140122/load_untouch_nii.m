...
function nii = load_untouch_nii(filename, img_idx, dim5_idx, dim6_idx, dim7_idx, ...
   ...
   if length(filename) > 2 & strcmp(filename(end-2:end), '.gz')
	 ...
	 ~strcmp(filename(end-6:end), '.nii.gz')
      ...
      if false%str2num(v(1:3)) < 7.1 | ~usejava('jvm')
	 ...
	 if (isunix)
	         [~,filenameout1,~]=fileparts(filename1);
		 system(['gunzip -c ' filename1 ' > ' tmpDir filesep filenameout1]);
	         filename1 = [tmpDir filesep filenameout1];
		 [~,filenameout2,~]=fileparts(filename2);
	         system(['gunzip -c ' filename2 ' > ' tmpDir filesep filenameout2]);
	         filename2 = [tmpDir filesep filenameout2];
	 else
	 ...
	 if (isunix)
      ...
      elseif strcmp(filename(end-6:end), '.nii.gz')
         ...
         gzFileName = filename;
	 if (isunix)
		 [~,filenameout,~]=fileparts(filename);
	         system(['gunzip -c ' filename ' > ' tmpDir filesep filenameout]);
		 filename = [tmpDir filesep filenameout];
	 else
